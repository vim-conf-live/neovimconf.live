...
const e = (tag: string, tw: string, ...children: any[]) => ({
  tag,
  props: { children, tw },
});
...
export const GET: APIRoute = async ({ request, params, cookies }) => {
  ...
  const { error, data: user } = await supabase
    .from("profiles")
    .select("username, full_name, job_description")
    .eq("ticket", id)
    .single();
  ...
  const html = e(
    "div",
    "w-full h-full flex flex-col text-5xl items-center justify-center relative bg-slate-900 text-teal-400",
    [
      e(
        "div",
        "absolute inset-12 border-4 flex flex-col border-teal-300 rounded-2xl bg-black p-8 mb-8",
        [
          ...
          e("div", "pl-4 flex flex-col text-teal-200", [
            ...
            e("p", "", [
              ...
              e(
                "span",
                "text-amber-400 ml-4 inline-block",
                ` "${user.username}"`,
              ),
            ]),
            e("p", "", [
              "job = ",
              e("span", "text-amber-400 ml-4", ` "${user.job_description}"`),
            ]),
            e("p", "text-", [
              e("span", "text-slate-600", "# aka: " + user.full_name),
            ]),
          ]),
        ],
      ),
